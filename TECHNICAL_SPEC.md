...
- **Require name:** when a poll has `require_name = true`, every voter must supply `voter_name` before the vote is accepted — signed-in voters included (their account name is prefilled client-side so it's low-friction, but a blank submission is still rejected rather than stored as null, which would otherwise surface as "Anonymous" when `show_voters` is on).
